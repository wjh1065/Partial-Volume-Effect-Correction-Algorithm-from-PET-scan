...
m = '1e-5_relu_epoch500.h5' # model name
...
def data_list_input(root):
    ...
    for i in dir_list:
        if i.startswith('ADNI'):
            ...
            list_input.append(data_path + '/method2_q99_gm_rPET.nii.gz')
    return list_input
...
def data_list_output(root):
    ...
    for i in dir_list:
        if i.startswith('ADNI'):
            data_path = os.path.join(root, i)
            list_output.append(data_path + '/method2_q99_pvc_rPET.nii.gz')
    return list_output
...
def lcs_4floor_Resunet3D(filters=6):
...
for j in tqdm(range(1,501)): # epoch
    ...
    history = model.fit(input_data_50, output_data_50, epochs=1, batch_size=4, shuffle=True, verbose=1, validation_data=(valid_input_data, valid_output_data))
    gc.collect()
    tf.keras.backend.clear_session()
    del input_data_50
    del output_data_50
